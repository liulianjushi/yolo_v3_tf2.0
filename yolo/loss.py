...
class YoloLoss(tf.keras.losses.Loss):
    ...
    def __get_scale_size(self, scale):
        return tf.convert_to_tensor([IMAGE_HEIGHT, IMAGE_WIDTH], dtype=tf.dtypes.float32) / get_coco_anchors(
            scale_type=scale)
    ...
    def __calculate_loss(self, y_true, y_pred):
        ...
        for i in range(self.scale_num):
            ...
            true_wh_offset = tf.keras.backend.switch(true_object_mask_bool, true_wh_offset,
                                                     tf.zeros_like(true_wh_offset))
            ...
            xy_loss = true_object_mask * box_loss_scale * self.__binary_crossentropy_keep_dim(true_xy_offset,
                                                                                              pred_features[..., 0:2],
                                                                                              from_logits=True)
            wh_loss = true_object_mask * box_loss_scale * 0.5 * tf.math.square(true_wh_offset - pred_features[..., 2:4])
            confidence_loss = true_object_mask * self.__binary_crossentropy_keep_dim(true_object_mask,
                                                                                     pred_features[..., 4:5],
                                                                                     from_logits=True) + (
                                          1 - true_object_mask) * self.__binary_crossentropy_keep_dim(true_object_mask,
                                                                                                      pred_features[...,
                                                                                                      4:5],
                                                                                                      from_logits=True) * ignore_mask
            class_loss = true_object_mask * self.__binary_crossentropy_keep_dim(true_class_probs,
                                                                                pred_features[..., 5:],
                                                                                from_logits=True)
